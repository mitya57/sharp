...
const cachePath = function () {
  console.log(`env.npm_config_cache = ${env.npm_config_cache}`);
  console.log(`env.APPDATA = ${env.APPDATA}`);
  ...
  console.log(`libvipsCachePath = ${libvipsCachePath}`);
...
const globalLibvipsVersion = function () {
  if (process.platform !== 'win32') {
    const cmdline = `PKG_CONFIG_PATH="${pkgConfigPath()}" pkg-config --modversion vips-cpp`;
    console.log(`Spawning: ${cmdline}`);
    console.log(`with options: ${spawnSyncOptions}`);
    const result = spawnSync(cmdline, spawnSyncOptions);
    console.log(`stdout: ${result.stdout}`);
    console.log(`stderr: ${result.stderr}`);
    const globalLibvipsVersion = result.stdout;
    /* istanbul ignore next */
